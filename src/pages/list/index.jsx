...
const List = ({ title = '', status = 0 }) => {

  const butt = () => {
    switch (status) {
      case 1:
        return [<span>finish reading</span>, ' →']
        break;
      case 2:
        return [<span>return in «to read»</span>, ' ↲']
        break;
      default:
        return [<span>start reading</span>, ' →']
        break;
    }
  };
  ...
  const setStatus = (id) => {
    ...
    let arr = localStorage.getItem('status-0');
    arr = arr ? JSON.parse(localStorage.getItem('status-0')) : [];
    console.log(arr)
    if(arr.length > 0) {
      arr.push(id)
    } else arr.push(id);
    console.log(arr)
    localStorage.setItem('status-0', JSON.stringify(arr))
  }

  const _arr = arr.map((el, i)=>{
    return <div key={i + '_card'} className='card'>
      <span>{el.author}</span>
      <div>
        <span onClick={(ev) => {
          ev.preventDefault();
          setStatus(el.id)
        }} className='but-status'>{butt()}</span>
        <h3>{el.title}</h3>
      </div>
      <p>{el.description}</p>
      <div className='tag-block'>
        {
          el.tags.map((tag, i_tag) => {
            return <button onClick={(ev) => {
              ev.preventDefault();
              setTag(tag)
            }} key={i_tag + '_tag'} className='tag'>#{tag}</button>
          })
        }
      </div>
    </div>;
  ...
  } else return <p className='empty-list'>List is empty</p>
};
